...
void handleButton1Press(){println("press 1"); myEmulador.press_KEY_B();  }//pressKey_B_p();
void handleButton1Release(){ println("release 1"); myEmulador.release_KEY_B(); }// pressKey_B_r();
...
void handleButton9Press(){println("press 9"); myEmulador.press_KEY_A(); }//pressKey_A_p();
void handleButton9Release(){ println("release 9"); myEmulador.release_KEY_A(); }//pressKey_A_r(); 
...
void joystick_walk()
{
  ...
  if(count>0 && !wasPressWalk)
  { 
      myEmulador.press_KEY_RIGHT(); 
      println("press left");  
      wasPressWalk = true; 
      lastTime = millis();   
      count = 0;
  ...
  if( (millis()-lastTime)>200 && wasPressWalk)
  {
    //pressKey_RIGHT_r();
    myEmulador.release_KEY_RIGHT();
    wasPressWalk = false;
